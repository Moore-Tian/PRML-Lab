...
def f0_to_coarse(f0):
  ...
  f0_coarse = f0_coarse * (f0_coarse > 0) + ((f0_coarse < 1) * 1) * (f0_coarse < f0_bin) + ((f0_coarse >= f0_bin) * (f0_bin - 1))
  return f0_coarse
...
def get_f0_predictor(f0_predictor,hop_length,sampling_rate,**kargs):
    from reference_tool.F0Predictor.RMVPEF0Predictor import RMVPEF0Predictor
...
def get_speech_encoder(speech_encoder,device=None,**kargs):
    if speech_encoder == "vec768l12":
        from reference_tool.vencoder.ContentVec768L12 import ContentVec768L12
        speech_encoder_object = ContentVec768L12(device = device)
    elif speech_encoder == "hubertsoft":
        from reference_tool.vencoder.HubertSoft import HubertSoft
        speech_encoder_object = HubertSoft(device = device)
    elif speech_encoder == "whisper-ppg":
        from reference_tool.vencoder.WhisperPPG import WhisperPPG
